// 12/11/24 Codewars 8 Kyu Fundamentals - Training JS #18: Methods of String object--concat() split() and its good friend join()
...
function splitAndMerge(string, separator) {
    ...
    const splitToChars = splitBySpaces.map(word => {
        return word.split('').join(separator);
    });
    return splitToChars.join(' ');
}

// 2nd attempt - not working, forgot "return" in map
function splitAndMerge(string, separator) {
    const splitBySpaces = string.split(' ');
    const splitToChars = splitBySpaces.map(word => {
...
// 1st attempt - not working, need to assign forEach to new variable or use map instead
function splitAndMerge(string, separator) {
    const splitString = string.split(' ');
    splitString.forEach(word => {
        word.split('');
    });
    return splitString.join(separator);
}
