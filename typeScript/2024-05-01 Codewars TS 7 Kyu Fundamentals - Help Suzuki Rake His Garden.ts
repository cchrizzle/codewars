// 5/1/25 Thursday Codewars TS 7 Kyu Fundamentals - Help Suzuki Rake His Garden!
...
export function rakeGarden(garden: string): string {
    const splitGarden = garden.split(' ');

    const rakedGarden = splitGarden.map(item => {
...
// 1st attempt - not working: for loop iterates over individual chars, not the words themselves.
export function rakeGarden(garden: string): string {
...
============= */
export function rakeGarden(garden:string):string {
    let whiteList = ['rock','gravel']
    return garden.split(' ').map((word)=> whiteList.includes(word) ? word : 'gravel').join(' ')
}